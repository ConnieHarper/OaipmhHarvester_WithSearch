...
class OaipmhHarvesterHarvestTable extends Omeka_Db_Table
{
     ...
     */
    public function findUniqueHarvest($baseUrl, $setSpec, $metadataPrefix, $identifier=null, $searchText=null)
    {
        ...
        if($identifier) 
            ...
            $select->where('identifier IS NULL');
        if($searchText) 
            $select->where('search = ?', $searchText);
        else
            $select->where('search IS NULL');
