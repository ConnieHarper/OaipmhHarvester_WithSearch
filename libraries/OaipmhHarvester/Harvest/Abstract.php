...
abstract class OaipmhHarvester_Harvest_Abstract
{
    ...
    private function _harvestRecords($resumptionToken = false)
    {
        ...
        if ($this->_harvest->identifier) {
        	$requestArguments = array('verb' => 'GetRecord');
                $requestArguments['identifier']  = $this->_harvest->identifier;
        } else {
	    ...
	    // skip over records that don't fit the search criteria if searching.
	    if ($this->_harvest->search){
                    $metadataPrefix = $this->_harvest->metadata_prefix;
                    $searchText = trim($this->_harvest->search);
