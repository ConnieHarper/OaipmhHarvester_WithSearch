...
class OaipmhHarvester_IndexController extends Omeka_Controller_Action
{
    ...
    public function listsAction()
    {
        ...
        try {
            ...
            $this->flash(print_r($baseUrl, true).print_r($requestArguments,true).$e->getMessage());
            $this->redirect->goto('index');
	}
        $availableMaps = array();
        if (isset($oaipmh->getOaipmh()->ListMetadataFormats)) {
        ...
        $search        = isset($_POST['search_text']) ? $_POST['search_text'] : null;
        ...
        $requestArguments['set'] = $setSpec;
        $requestArguments['metadataPrefix'] = $metadataPrefix;
        // If a resumption token exists, process it. For a data provider that 
        // uses a resumption token for sets, see: http://www.ajol.info/oai/
        if (isset($_POST['resumption_token'])) {
            $requestArguments['resumptionToken'] = $_POST['resumption_token'];
        ...
        // If no error was returned, it is a valid ListSets response.
            ...
            $records = $oaipmh->getOaipmh()->ListRecords;
        }

        
        // Set the resumption token, if any.
        if (isset($oaipmh->getOaipmh()->ListRecords->resumptionToken)) {
            $resumptionToken = $oaipmh->getOaipmh()->ListRecords->resumptionToken;
        } else {
            $resumptionToken = false;
        }
       $goodRecords=array(); 
       foreach ($records->record as $record) {
       		$recordDc = @ $record->metadata->children($metadataPrefix, true)->children('dc', true); 
		$title = substr($recordDc->title[0],0);

		//Find a title that has your search keyword in it
		if (!(strpos($title, $search)===false)){
			array_push($goodRecords,$record);
		}
	}
        // Set the variables to the view object.
        $this->view->records     = $goodRecords;
        $this->view->resumptionToken = $resumptionToken;
        $this->view->baseUrl         = $baseUrl;
        $this->view->availableMaps   = $availableMaps;
        $this->view->metadataClass = $metadataClass;
        $this->view->metadataPrefix   = $metadataPrefix;
        $this->view->setSpec   = $setSpec;
        $this->view->searchText   = $search;
        $this->view->setName   = $setName;
    }
    /**
    ...
    public function harvestAction()
    {
        ...
        $metadataClass   = '';
        $metadataSpec   = '';
	if (isset ($_POST['metadata_prefix'])){
        	$metadataPrefix = $_POST['metadata_prefix'];
        	$metadataClass = $_POST['metadata_class'];
	} else if(isset($_POST['metadata_spec'])){
        ...
        $setSpec        = isset($_POST['set_spec']) ? $_POST['set_spec'] : null;
        $setName        = isset($_POST['set_name']) ? $_POST['set_name'] : null;
        $recordIdentifier        = isset($_POST['record_identifier']) ? $_POST['record_identifier'] : null;
        $setDescription = isset($_POST['set_description']) ? $_POST['set_description'] : null;
        $search        =  isset($_POST['search_text']) ? $_POST['search_text'] : null;
        ...
        if($harvest_id) {
            ...
            $harvest = $this->getTable('OaipmhHarvesterHarvest')->findUniqueHarvest($baseUrl, $setSpec, $metadataPrefix, $recordIdentifier, $search);
            ...
            if(!$harvest) {
                ...
                $harvest->identifier  = $recordIdentifier;
                $harvest->search  = $search;
            }
        }
